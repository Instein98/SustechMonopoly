...
public class CamRotate : MonoBehaviour {
	...
	void Update () {
		camera.LookAt(center.transform);
		// Debug.Log(GameController.hasUI);
		...
		if (down){
			...
			//按下期间鼠标是否移动
			if (!GameController.hasUI && nowPos != latePos && change){
		...
		// Zoom In
		if (!GameController.hasUI && Input.GetAxis("Mouse ScrollWheel") > 0){
        ...
        if(!GameController.hasUI && Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved){
            Vector2 touchDelPos = Input.GetTouch(0).deltaPosition;
			center.Rotate(0, touchDelPos.x / 10, 0, Space.World);
			if(angleX >= 30 && angleX < 200){ // only down
				if(touchDelPos.y > 0){
					center.Rotate(-touchDelPos.y / 8, 0, 0, Space.Self);
				}
			} else if(angleX <= 330 && angleX > 201){ // only up
				if(touchDelPos.y < 0){
					center.Rotate(-touchDelPos.y / 8, 0, 0, Space.Self);
        ...
        }else if(!GameController.hasUI && Input.touchCount >= 2){
			//多点触摸, 放大缩小
