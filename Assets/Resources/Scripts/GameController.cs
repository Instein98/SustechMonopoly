...
// *************************************
	...
	void Start () {
		// Debug.Log("GameController birth!!");
		gamePlayers.Clear();
		...
		NetManager.getInstance().RegistProtocoal("GAMEOVER",delegate(ProtocolBase p){
			...
			messageBox.transform.localScale *= 0.8f;
			string[] desc = p.GetDesc().Split(',');
		...
		NetManager.getInstance().RegistProtocoal("QUERY",delegate(ProtocolBase p){
			...
			double temp = (clientPlayer.credit + clientPlayer.intell*0.05)*4/(2.75+o*6);
		...
		GlobalControl.music.volume = GlobalControl.volume;
	}
